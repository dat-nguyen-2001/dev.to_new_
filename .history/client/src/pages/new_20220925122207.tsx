...
const New = () => {
    ...
    const displayTagsHandler = () => { setDisplayTags(true) };
    const tagsInput = document.que
            ...
            <div className='flex flex-col content-center h-screen col-span-3 lg:col-span-5'>
                ...
                <div className='overflow-y-auto bg-white h-[82vh] border-[2px] rounded-lg flex flex-col py-3 md:py-10 px-2 md:px-16 space-y-5 lg:ml-[100px]'>
                    ...
                    <form className='relative'>
                        <input onFocus={displayTagsHandler} placeholder='Add up to 4 tags...' className='w-full outline-none tagsInput' />
                        <div className={`${displayTags ? "flex" : "hidden"}  z-[100] absolute top-10 flex-col px-2 bg-white w-full border-[.1em] border-solid border-[#777] rounded-md max-h-[500px] overflow-y-auto`}>
                            <h2 className='text-lg font-bold pt-3 pb-3 px-3'>Top tags</h2>
                            <hr></hr>
                            <ul className='flex flex-col space-y-0 cursor-pointer pt-3'>
                                {tags.map(tag => (
                                    <li key={tag} className='py-5 px-3 hover:text-[#3e45b2] hover:bg-[#f5f5f5]'>#{tag}</li>
                                ))}
                            </ul>
                        </div>
                    </form>
