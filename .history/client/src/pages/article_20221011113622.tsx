...
const ArticlePage = () => {
    ...
    const [listed_username, setListedUsername] = useState<string[]>([])
    ...
    useEffect(() => {
        username !== undefined && getArticlesByUser(username).then(data => {
            ...
            // Set listed_username
            setListedUsername(filteredArticle[0].listed_users.map((user: any) => user.username))
        })
    }, [])
    ...
    const savedArticle = listed_username.includes(useRecoilValue(usernameState));
    const [isSaved, setIsSaved] = useState<Boolean>(savedArticle)
