        ...
        <div className='md:grid grid-cols-4 bg-[#f5f5f5] lg:grid-cols-7 2xl:pl-[100px]'>
            <div className='flex flex-col content-center h-screen col-span-3 lg:col-span-5'>
                ...
                <div className='overflow-y-auto bg-white h-[82vh] border-[2px] rounded-lg flex flex-col py-3 md:py-10 px-2 md:px-16 space-y-5 lg:ml-[100px]'>
                    ...
                    {/* Tags */}
                    {displayTags ? <form className='relative'>
                        <input placeholder='Add up to 4 tags...' className='w-full outline-none' onBlur={() => setDisplayTags(false)} onClick={() => setDisplayTags(true)}/>
                        <div className={`${displayTags ? "flex" : "hidden"} z-10 absolute top-10 flex-col px-2 bg-white w-full border-[.1em] border-solid border-[#777] rounded-md max-h-[500px] overflow-y-auto`}>
                            <h2 className='text-lg font-bold pt-3 pb-3 px-3'>Top tags</h2>
                            <hr></hr>
                            <ul className='flex flex-col space-y-0 cursor-pointer pt-3' onClick={() => console.log('HAHA')}>
                                {tags.map(tag => (
                                        <li key={tag} className='py-5 px-3 hover:text-[#3e45b2] hover:bg-[#f5f5f5]' onClick={() => console.log(tag)}>#{tag}</li>
                                ))}
                            </ul>
                        </div>
                    </form>}
