...
function User() {
  ...
  const uploadImage = () => {
    ...
    axios.post('https://api.cloudinary.com/v1_1/dfqafsyni/image/upload', formData).then((res: any) => { res.data.url ? setUrl(res.data.url) : setUrl("") }).then(changeProfilePicture(sessionStorage))
  }
  useEffect(() => {
    if (files !== null) {
      uploadImage();
