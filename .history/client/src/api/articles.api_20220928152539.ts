...
const createArticlesByUser = async function (username: string) {
    await axios.post(`http://localhost:4000/articles/${username}`, {title, content, tags, coverImage}, {headers: {Authorization: `Bearer ${sessionStorage.getItem('accessToken')}`}})
}
