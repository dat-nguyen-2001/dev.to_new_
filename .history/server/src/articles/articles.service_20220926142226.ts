...
export class ArticlesService {
    ...
    async createArticle(req: Request, createArticleDto: CreateArticleDto) {
        ...
        // Extract the request body to get user's email || Refactor later - split it into a separate function
        ...
        newArticle.user_id = await User.findOne({where: {email}}).then(user => user.id);
        newArticle.reactions = 0;
        newArticle.comments = [];
        newArticle.listed_userIds = [];
