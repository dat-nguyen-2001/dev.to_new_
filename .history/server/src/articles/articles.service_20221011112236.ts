...
export class ArticlesService {
    ...
    async saveArticle(req: Request, id: number) {
        //Extract email from the access token   
        const authorizationHeader = req.headers['authorization'] === undefined ? req.body.headers.Authorization : ;
        const token = authorizationHeader.split(' ')[1];
        ...
        const article = await Article.findOne({ where: { id }, relations: {listed_users: true} });
